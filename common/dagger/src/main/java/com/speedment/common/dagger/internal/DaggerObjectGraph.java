...
public class DaggerObjectGraph extends ObjectGraph {
    ...
    public static ObjectGraph makeGraph(DaggerObjectGraph base, Loader plugin, Object... modules) {
        final Map<String, Class<?>> injectableTypes = new LinkedHashMap<>();
        final Map<Class<?>, StaticInjection> staticInjections
            = new LinkedHashMap<>();

        final StandardBindings baseBindings = (base == null)
            ? new StandardBindings()
            : new StandardBindings(base.setBindings);

        final BindingsGroup overrideBindings = new OverridesBindings();

        final Map<ModuleAdapter<?>, Object> loadedModules = Modules.loadModules(plugin, modules);
        for (Map.Entry<ModuleAdapter<?>, Object> loadedModule : loadedModules.entrySet()) {

            @SuppressWarnings("unchecked")
            final ModuleAdapter<Object> moduleAdapter = (ModuleAdapter<Object>) loadedModule.getKey();
            ...
            try {
                final BindingsGroup addTo = moduleAdapter.overrides ? overrideBindings : baseBindings;
    ...
    /**
     * @param classLoader the {@code ClassLoader} used to load dependent
     * bindings.
     * @param injectableKey the key used to store the injectable type. This is a
     * provides key for interfaces and a members injection key for other types.
     * That way keys can always be created, even if the type has no injectable
     * constructor.
