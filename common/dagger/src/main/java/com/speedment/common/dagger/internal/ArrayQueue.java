...
public class ArrayQueue<E> extends AbstractCollection<E>
    ...
    private boolean delete(int i) {
        //checkInvariants();
        final Object[] elmnts = this.elements;
        final int mask = elmnts.length - 1;
        final int h = head;
        final int t = tail;
        ...
        if (front < back) {
            if (h <= i) {
                System.arraycopy(elmnts, h, elmnts, h + 1, front);
            } else { // Wrap around
                System.arraycopy(elmnts, 0, elmnts, 1, i);
                elmnts[0] = elmnts[mask];
                System.arraycopy(elmnts, h, elmnts, h + 1, mask - h);
            }
            elmnts[h] = null;
            head = (h + 1) & mask;
            return false;
            ...
            if (i < t) { // Copy the null tail as well
                System.arraycopy(elmnts, i + 1, elmnts, i, back);
                tail = t - 1;
            } else { // Wrap around
                System.arraycopy(elmnts, i + 1, elmnts, i, mask - i);
                elmnts[mask] = elmnts[0];
                System.arraycopy(elmnts, 1, elmnts, 0, t);
                tail = (t - 1) & mask;
            }
            return true;
        }
    ...
    public <T> T[] toArray(T[] a) {
        ...
        if (a.length < size) {
            ...
            final T[] a2 = (T[]) java.lang.reflect.Array.newInstance(
    ...
    public ArrayQueue<E> clone() throws CloneNotSupportedException {
        try {
            @SuppressWarnings("unchecked")
            final ArrayQueue<E> result = (ArrayQueue<E>) super.clone();
