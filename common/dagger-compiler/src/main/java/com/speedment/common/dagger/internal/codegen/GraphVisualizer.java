...
public final class GraphVisualizer {
    ...
    private Map<Binding<?>, String> buildNamesIndex(Map<String, Binding<?>> bindings) {
        ...
        final Map<String, Binding<?>> shortNameToBinding = new TreeMap<>();
        final Set<Binding<?>> collisions = new HashSet<>();
        
        for (final Map.Entry<String, Binding<?>> entry : bindings.entrySet()) {
            final String key = entry.getKey();
            final Binding<?> binding = entry.getValue();
            final String shortName = shortName(key);
            final Binding<?> collision = shortNameToBinding.put(shortName, binding);
        ...
        for (final Map.Entry<String, Binding<?>> entry : shortNameToBinding.entrySet()) {
    ...
    String shortName(String key) {
        ...
        final String annotationSimpleName = matcher.group(1);
        
        if (annotationSimpleName != null) {
            result.append('@').append(annotationSimpleName).append(' ');
        }

        final String simpleName = matcher.group(2);
        result.append(simpleName);

        final String typeParameters = matcher.group(3);
        if (typeParameters != null) {
            result.append(typeParameters);
        }

        final String arrays = matcher.group(4);
        if (arrays != null) {
    ...
    /**
     * A Comparator for Bindings so we can insure a consistent ordering of
     * output.
     */
